...
contextBridge.exposeInMainWorld('wisadelDesktop', {
  ...
  detectStableDiffusion: () => ipcRenderer.invoke('wisadel:detect-stable-diffusion') as Promise<{ installed: boolean; root?: string; running: boolean; endpoint?: string; hardware?: { platform: 'nvidia' | 'amd' | 'cpu'; name: string; memoryMb?: number } }>,
  installStableDiffusion: () => ipcRenderer.invoke('wisadel:install-stable-diffusion') as Promise<{ root: string; hardware: { platform: 'nvidia' | 'amd' | 'cpu'; name: string; memoryMb?: number } }>,
  startStableDiffusion: (root: string) => ipcRenderer.invoke('wisadel:start-stable-diffusion', root) as Promise<{ endpoint: string }>,
  onStableDiffusionInstall: (callback: (event: { type: string; percent?: number; root?: string; message?: string }) => void) => { const listener = (_event: Electron.IpcRendererEvent, payload: { type: string; percent?: number; root?: string; message?: string }) => callback(payload); ipcRenderer.on('wisadel:sd-install', listener); return () => ipcRenderer.removeListener('wisadel:sd-install', listener); },
