...
function Conversation() {
  ...
  return <section className="conversation">
    <header className="conversation-header"><div><span>{page === 'chat' ? 'AI 对话' : '图像生成'}</span><h2>{active?.title ?? '正在载入'}</h2></div><button className="model-selector"><span className="model-status" />{page === 'chat' ? 'DeepSeek' : 'Qwen Image'}<ChevronDown size={15} /></button></header>
