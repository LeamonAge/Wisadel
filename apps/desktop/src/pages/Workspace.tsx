...
function WorkspaceTrustMenu() {
  ...
  const inspect = async () => { if (active.trust !== 'TRUSTED') return; const next = await window.wisadelDesktop?.workspaceContext(active.path); if (next) setContext(next); };
  return <>{context && <div className="modal-backdrop" onMouseDown={() => setContext(null)}><section className="workspace-context-dialog" onMouseDown={(event) => event.stopPropagation()}><header><div><span>LOCAL WORKSPACE</span><h2>{active.name}</h2></div><button className="icon-button" onClick={() => setContext(null)} title="关闭"><X size={18} /></button></header><div className="workspace-context-grid"><div><strong>Git</strong><p>{context.git.branch}</p><pre>{context.git.status || '工作区无改动'}</pre></div><div><strong>语言</strong><p>{context.project.languages.join(' · ') || '未识别'}</p><strong>建议命令</strong>{context.project.suggestedCommands.map((command) => <code key={command}>{command}</code>)}</div></div><div className="workspace-files">{context.tree.slice(0, 120).map((item) => <span key={item.path}>{item.kind === 'directory' ? '▸ ' : '  '}{item.path}</span>)}</div></section></div>}<button className={`workspace-trust-badge ${active.trust.toLowerCase()}`} onClick={() => void api.trustWorkspace(active.id, active.trust === 'TRUSTED' ? 'UNTRUSTED' : 'TRUSTED').then((next) => setItems((all) => all.map((item) => item.id === next.id ? next : item)))} onDoubleClick={() => void inspect()} title={active.trust === 'TRUSTED' ? '单击切换信任状态，双击查看本机工程摘要' : '单击设为已信任'}><span>{active.name}</span><small>{active.trust === 'TRUSTED' ? '已信任' : '待信任'}</small></button></>;
}
