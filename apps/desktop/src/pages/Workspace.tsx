...
function WorkspaceTrustMenu() {
  ...
  const [context, setContext] = useState<{ project: { languages: string[]; suggestedCommands: string[] }; git: { branch: string; status: string }; tree: Array<{ path: string; kind: 'file' | 'directory' }> } | null>(null);
  ...
  const inspect = async () => { if (active.trust !== 'TRUSTED') return; const next = await window.wisadelDesktop?.workspaceContext(active.path); if (next) { setContext(next); setSelectedFile(null); setShowDiff(false); } };
  const openFile = async (relativePath: string) => { setLoadingFile(true); setSaveError(''); try { const content = await window.wisadelDesktop?.agentReadFile(active.path, relativePath); if (content == null) return; setSelectedFile(relativePath); setOriginalContent(content); setEditedContent(content); } catch (error) { setSaveError(error instanceof Error ? error.message : String(error)); } finally { setLoadingFile(false); } };
  const saveFile = async () => { if (!selectedFile || editedContent === originalContent) return; setSaveError(''); try { await window.wisadelDesktop?.agentWriteFile(active.path, selectedFile, editedContent); setOriginalContent(editedContent); setShowDiff(false); const next = await window.wisadelDesktop?.workspaceContext(active.path); if (next) setContext(next); } catch (error) { setSaveError(error instanceof Error ? error.message : String(error)); } };
  const diffLines = (() => { const before = originalContent.split('\n'); const after = editedContent.split('\n'); const max = Math.max(before.length, after.length); return Array.from({ length: max }, (_, index) => ({ before: before[index] ?? '', after: after[index] ?? '', changed: before[index] !== after[index] })); })();
  return <>{context && <div className="modal-backdrop" onMouseDown={() => setContext(null)}><section className="workspace-context-dialog workspace-editor-dialog" onMouseDown={(event) => event.stopPropagation()}><header><div><span>LOCAL WORKSPACE</span><h2>{active.name}</h2></div><button className="icon-button" onClick={() => setContext(null)} title="关闭"><X size={18} /></button></header><div className="workspace-context-grid"><div><strong>Git</strong><p>{context.git.branch}</p><pre>{context.git.status || '工作区无改动'}</pre></div><div><strong>语言</strong><p>{context.project.languages.join(' · ') || '未识别'}</p><strong>建议命令</strong>{context.project.suggestedCommands.map((command) => <code key={command}>{command}</code>)}</div></div><div className="workspace-editor-layout"><div className="workspace-files">{context.tree.filter((item) => item.kind === 'file').slice(0, 160).map((item) => <button className={selectedFile === item.path ? 'active' : ''} key={item.path} onClick={() => void openFile(item.path)}>{item.path}</button>)}</div><div className="workspace-editor"><div className="workspace-editor-toolbar"><strong>{selectedFile ?? '选择文件查看'}</strong>{selectedFile && <><span>{editedContent === originalContent ? '未修改' : '已修改'}</span><button disabled={editedContent === originalContent || loadingFile} onClick={() => setShowDiff(true)}>审阅差异</button></>}</div>{selectedFile && <textarea value={editedContent} onChange={(event) => setEditedContent(event.target.value)} spellCheck={false} disabled={loadingFile} />}{saveError && <p className="workspace-editor-error">{saveError}</p>}</div></div>{showDiff && selectedFile && <div className="workspace-diff"><header><strong>保存前差异</strong><button className="icon-button" onClick={() => setShowDiff(false)} title="关闭"><X size={16} /></button></header><pre>{diffLines.map((line, index) => <span className={line.changed ? 'changed' : ''} key={index}>{line.changed ? `- ${line.before}\n+ ${line.after}` : `  ${line.after}`}\n</span>)}</pre><footer><button onClick={() => setShowDiff(false)}>取消</button><button className="primary" onClick={() => void saveFile()}>确认保存</button></footer></div>}</section></div>}<button className={`workspace-trust-badge ${active.trust.toLowerCase()}`} onClick={() => void api.trustWorkspace(active.id, active.trust === 'TRUSTED' ? 'UNTRUSTED' : 'TRUSTED').then((next) => setItems((all) => all.map((item) => item.id === next.id ? next : item)))} onDoubleClick={() => void inspect()} title={active.trust === 'TRUSTED' ? '单击切换信任状态，双击查看本机工程摘要' : '单击设为已信任'}><span>{active.name}</span><small>{active.trust === 'TRUSTED' ? '已信任' : '待信任'}</small></button></>;
}
