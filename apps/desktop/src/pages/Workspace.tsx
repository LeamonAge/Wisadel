...
function SettingsDialog() {
  ...
  if (!open) return null;
  return <div className="modal-backdrop" onMouseDown={() => close(false)}><section className="settings-dialog" onMouseDown={(event) => event.stopPropagation()}><header><div><span>偏好设置</span><h2>Wisadel 设置</h2></div><button className="icon-button" onClick={() => close(false)}><X size={19} /></button></header><div className="settings-content"><nav><button className="active"><Settings size={17} />常规</button><button><CircleUserRound size={17} />个性化</button><button><SlidersHorizontal size={17} />高级</button></nav><div className="settings-panel"><h3>常规</h3><div className="setting-row"><div><strong>界面主题</strong><span>主题会保存在当前设备。</span></div><div className="theme-switch" role="group" aria-label="界面主题"><button className={theme === 'dark' ? 'active' : ''} onClick={() => setTheme('dark')}>深色</button><button className={theme === 'light' ? 'active' : ''} onClick={() => setTheme('light')}>浅色</button></div></div><div className="setting-row"><div><strong>自动生成</strong><span>关闭后，每次生成前需要手动确认</span></div><button className="toggle" aria-label="自动生成开关"><i /></button></div><div className="setting-row"><div><strong>本地缓存</strong><span>保留最近会话，断网时仍可浏览</span></div><button className="toggle on" aria-label="本地缓存开关"><i /></button></div><div className="setting-note">自定义 API、本地 SD 与自动更新将在后续版本开放。</div></div></div></section></div>;
}
